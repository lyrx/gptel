...
(defun gptel-org-merge-tests-run ()
  ...
  ;; Integration: stub heading props; `gptel-org--send-with-props' must merge
  ;; org vs buffer (not plain `or`).
  (let (captured)
  ...
  ;; Section at EOF overrides GPTEL_SYSTEM and merge (requires section support).
    ...
    (cl-letf (((symbol-function 'gptel-org--entry-properties)
      ...
      (with-temp-buffer
        ...
        (setq-local gptel--system-message "from-buf"
                    ...
                    gptel-tools nil
                    gptel-org--send-system-state nil
                    gptel-org--system-section-cache nil)
        (gptel-org--send-with-props
         (lambda (&rest _)
           (setq captured gptel--system-message)))
        (gptel-org-merge-tests--assert "section wins over org and buffer merge"
          captured "From section.")))))
